...
from lsm.pyNN.utils import p2n
from lsm.utils import windowed_events


def create_iaf_psc_exp(n_E, n_I):
    nodes = nest.Population(n_E + n_I,
                            nest.native_cell_type('iaf_psc_exp')(**{
                                'C_m': 30.0,  # 1.0,
...
def connect_tsodyks(nodes_E, nodes_I):
    ...
    def connect(src, trg, J, n_syn, syn_param):
        # TODO how can we set a weight distribution when working with native synapse types?
        # a 'float' is expected even though supplying a 'dict' should be valid for NEST
        #
        # connector = nest.FixedNumberPreConnector(n_syn)
        # synapse = nest.native_synapse_type("tsodyks_synapse")(**{
        #             'delay': delay,
        #             'weight': {"distribution": "normal_clipped", "mu": J, "sigma": 0.7 * abs(J),
        #                        "low" if J >= 0 else "high": 0.
        #                        },
        #             **syn_param})
        # nest.Projection(src, trg, connector, synapse_type=synapse)
        nest.nest.Connect(p2n(src), p2n(trg),
...
def inject_noise(nodes_E, nodes_I):
    ...
    noise = nest.Population(1, nest.native_cell_type('poisson_generator')(**{'rate': p_rate}))

    nest.nest.CopyModel('static_synapse_hom_w', 'excitatory_noise', {'weight': J_noise})
    nest.nest.Connect(p2n(noise), p2n(nodes_E, nodes_I), syn_spec={'model': 'excitatory_noise', 'delay': delay})

    # TODO
    # - CopyModel does not make the model available to pyNN
    # - Without CopyModel, static_synapse_hom_w can not be used, as weight is static
    # synapse = nest.native_synapse_type('excitatory_noise')
    # nest.Projection(noise, nodes_E, nest.AllToAllConnector(), synapse_type=synapse(weight=J_noise))
    # nest.Projection(noise, nodes_I, nest.AllToAllConnector(), synapse_type=synapse(weight=J_noise))
...
class LSM(object):
    def __init__(self, n_exc, n_inh, n_rec,
        ...
        # self.rec_nodes.record('spikes')
        self._rec_detector = nest.nest.Create('spike_detector', 1)
        nest.nest.Connect(p2n(self.rec_nodes), self._rec_detector)

    def get_states(self, times, tau):
        # TODO the following fails because of some unknown reason (too much native?)
        # spike_times = self.rec_nodes.get_data().segments[0].spiketrains[0]
