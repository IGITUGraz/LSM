...
def inject_spikes(inp_spikes, neuron_targets):
    spike_generators = nest.Population(len(inp_spikes), nest.SpikeSourceArray(spike_times=inp_spikes))
    ...
    nest.nest.Connect(p2n(spike_generators), p2n(neuron_targets),
    ...
    # TODO again: how to supply dicts as parameters via the native interface?
    # nest.Projection(spike_generators, neuron_targets,
    #                 nest.FixedNumberPostConnector(C_inp),
    #                 nest.native_synapse_type('static_synapse')(**{
    #                    'delay': delay,
    #                    'weight': {'distribution': 'uniform',
    #                          'low': 2.5 * 10 * 5.0,
    #                          'high': 7.5 * 10 * 5.0}
...
def main():
    nest.nest.SetKernelStatus({'print_time': True, 'local_num_threads': 11})
